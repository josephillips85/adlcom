...
static short get_com_port(int i) {
  return *(short __far *)MK_FP(0x40, 2 * i - 2);
}


static void usage(void) {
  cputs("Usage: ADLCOM [COM1|COM2|COM3|COM4] [OPL3] [[FAKE]BLASTER[=220]]\r\n"
        "       ADLCOM STATUS\r\n"
        "       ADLCOM UNLOAD\r\n");
}
...
static void status(struct config __far *cfg) {
  ...
  //See port_init and baud rate divisor.
  cputs("  Baud Rate: 115200");
  cputs("\r\n");
...
int main(void) {
  ...
  cputs("ADLCOM " XSTR(VERSION_MAJOR) "." XSTR(VERSION_MINOR)
  ...
  for (i = 0; i < 256; ++i) {
    ...
    if (result == 0 && !found_unused_amis_id) {
      ...
      if (info.version.word != (VERSION_MAJOR * 256 + VERSION_MINOR)) {
        cputs("Error: A different version of ADLCOM is already loaded.\r\n");
        return 1;
      }
      resident =
  ...
  if (mode == MODE_UNLOAD) {
    if (!resident) {
      cputs("ADLCOM is not loaded.\r\n");
      return 1;
    } else if (uninstall(resident)) {
      cputs("ADLCOM is now unloaded from memory.\r\n");
      return 0;
    } else {
      cputs("Could not unload ADLCOM.\r\n");
      return 1;
    }
  }
  ...
  if (resident) {
    cputs("ADLCOM was already loaded.\r\n\r\n");
    status(resident);
    return 1;
  }
