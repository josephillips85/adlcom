...
static void in_place_mode(const char *orig_filename) {
  ...
  printf("AdlCom Patcher (BETA)" XSTR(VERSION) "\n");
  printf("https://www.github.com/josephillips85/adlcom" "\n");
  printf("\n");
  printf("Patching %s\n", orig_filename);
...
static void usage() {
  fprintf(stderr,
          "AdlCom Patcher (BETA)" XSTR(VERSION) "\n"
          "https://www.github.com/josephillips85/adlcom" "\n"
          "\n"
          "Usage: adcpatch [OPTIONS...] FILE\n"
          "       adcpatch [OPTIONS...] -o OUTPUT-FILE INPUT-FILE\n"
          "\n"
          "Options:\n"
          "  -p PORT       Set COM port (default: " DEFAULT_PORT ")\n"
          );
  exit(1);
}
...
static short parse_port(const char *arg) {
  int i, port;
  if (strncasecmp(arg, "com", 3) == 0 && (i = arg[3] - '0') >= 1 && i <= 3) {
    port = get_com_port(i);
  } else {
